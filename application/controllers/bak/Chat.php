...
class Chat extends CI_Controller {
	...
	public function index()
	{
		...
		$data['admin'] = $this->M_database->getWhere('t_asisten', 't_jabatan', 't_asisten.jabatan_id=t_jabatan.jabatan_id', array('status' => 1, 't_asisten.jabatan_id' => 6), 'asisten_nama ASC');		

        $this->template->load('bak/template-bak', 'bak/v-chat', $data);
	}
	...
	function cek_pesan()
	{
		...
		$addPesanMhs = array_push($isi, $jmlTicket + $jmlPesanMhs);
		...
		if ($last_msg) {			
			...
			$last_msg['link'] = base_url().'bak/chat?'.$last_msg['pesan_pengirim'];
		...
		foreach ($notif as $key => $value) {
			...
			$isi[1][$key]['link'] = base_url().'bak/chat?'.$pengirim;
		}				
