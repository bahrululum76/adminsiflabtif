...
class Chat extends CI_Controller {
	...
	public function index()
	{
		...
		$data['user'] = $this->db->where('dosen_id', $this->session->userdata('dosen_id'))->get('t_dosen')->row();
		$data['tabTitle'] = 'Pesan';
        ...
        $this->template->load('dosen/template-dosen', 'dosen/v-chat', $data);
	...
	function cek_pesan()
	{
		...
		if ($last_msg) {			
			...
			$last_msg['link'] = base_url().'dosen/chat?'.$last_msg['pesan_pengirim'];
		...
		foreach ($notif as $key => $value) {
			...
			$isi[1][$key]['link'] = base_url().'dosen/chat?'.$pengirim;
		}				
