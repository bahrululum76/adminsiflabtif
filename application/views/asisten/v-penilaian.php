...
<div class="col s12">
	<div class="main-wrapper">
		<div class="top-panel right-align">
			<span class="panel-title-1 left" style="margin-left: 36px; position: relative; top: 4px">Penilaian Praktikum</span>
						...
						<select class="select2-no-search" onchange="location = this.value;document.querySelector('.mypage-loader').classList.remove('hide');">
							...
							<?php 
                                    foreach ($jadwal->result() as $key => $value):
                                        ...
                                        ?>
							<option <?= $selected ?>
								value="<?= base_url() ?>asisten/penilaian?idp=<?= $value->jadwal_kode ?>">
					...
					<table class="highlight datatable-nopage">
						<thead>
							<tr>
								...
								<th width="130px">NPM</th>
								<th>Nama Mahasiswa</th>
								<th class="center" width="90px">Total Tugas</th>
								<th class="center" width="90px">Kehadiran<br><?= $jadwal_ini->p_kehadiran ?> %</th>
								<th class="center" width="90px">Tugas<br><?= $jadwal_ini->p_tugas ?> %</th>
								<th class="center" width="90px">Ujian<br><?= $jadwal_ini->p_ujian ?> %</th>
						...
						<tbody>
							...
							<tr>
								...
								<td><?= $value->npm ?></td>
								<td><?= $value->nama ?></td>
								<td class="center" id="tugas<?= $value->npm ?>"><?= $nilai ?></td>
								<td class="center kehadiran<?= $value->npm ?>"><?= $total_kehadiran ?></td>
								<td class="td-nilai-ujian td-data center" id="td<?= $value->npm ?>" data-npm="<?=$value->npm?>">
									<span class="text-nilai txt-n-tugas<?= $value->npm ?> muncul"><?= $value->nilai_tugas ?></span>
									<input type="number" value="<?= $value->nilai_tugas ?>" data-npm="<?= $value->npm ?>" class="input-number input-nilai inp-n-tugas<?= $value->npm ?> hilang browser-default" min="0" max="100" maxlength="3" step="10" title="Max nilai 100">
								</td>
								<td class="td-nilai-ujian td-data center" data-npm="<?=$value->npm?>">
									<span class="text-nilai txt-n-ujian<?= $value->npm ?> muncul"><?= $value->nilai_ujian ?></span>
									<input type="number" value="<?= $value->nilai_ujian ?>" class="input-number input-nilai inp-n-ujian<?= $value->npm ?> hilang browser-default" min="0" max="100" maxlength="3" step="10" title="Max nilai 100">
